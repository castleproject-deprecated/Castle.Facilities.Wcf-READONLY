...
namespace Castle.Facilities.WcfIntegration.Client.Async.TypeSystem
{
	...
	{
		private static int currentToken = typeof(object).GetConstructors()[0].MetadataToken;
		private static ModuleHandle moduleHandle = typeof(object).Module.ModuleHandle;
		...
		{
			return moduleHandle.GetRuntimeMethodHandleFromMetadataToken(currentToken++);
		}
	}
}